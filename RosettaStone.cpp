...
Map<string, double> kGramsIn(const string& str, int kGramLength) {
    ...
    if(kGramLength <= 0) error("The KGramlength parameter is not positive");

    unsigned length = str.length();
    for(int i = 0 ; i + kGramLength <= length ;i ++){
...
Map<string, double> normalize(const Map<string, double>& input) {
    ...
    for(string key : input){
        sum += pow(input[key] , 2);
        if(sum == 0) error("The input does not contain any nonzero values");
    }
...
Map<string, double> topKGramsIn(const Map<string, double>& source, int numToKeep) {
    ...
    if(numToKeep < 0){
        error("The numToKeep is negative");
    }
    ...
    for(int i = 0 ; i + numToKeep < size ; i ++){
        pq.dequeue();
    }
    while(!pq.isEmpty()){
...
double cosineSimilarityOf(const Map<string, double>& lhs, const Map<string, double>& rhs) {
   double similarity = 0;
   for(string key : lhs){
       if(rhs.containsKey(key)) similarity += lhs[key] * rhs[key];
   }
   return similarity;
}
...
string guessLanguageOf(const Map<string, double>& textProfile,
    ...
    if(corpora.isEmpty()) error("The set of corpora is empty");

    double maxSimilarity = 0 , temp;
    string name = "";
    for(Corpus corpus : corpora){
        temp = cosineSimilarityOf(textProfile,corpus.profile);
