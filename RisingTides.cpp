...
void flow (const Grid<double> & terrian ,Grid<bool> &res ,Queue<GridLocation>& queue, GridLocation location,double height){
    if(terrian.inBounds(location) && terrian[location] <= height && res[location] == false){
        res[location] = true;
...
Grid<bool> floodedRegionsIn(const Grid<double>& terrain,
    ...
    for(GridLocation location : sources){
        res.set(location.row , location.col , true);
        queue.enqueue(location);
    }
    ...
    while(!queue.isEmpty()){
        ...
        flow(terrain ,res , queue, up , height);
        flow(terrain, res, queue,down ,height);
        flow(terrain , res , queue, left , height);
        flow(terrain, res , queue, right, height);
    }
    return res;
}
